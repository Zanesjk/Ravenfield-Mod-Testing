...
public class SanityCheck {
	...
	public static bool DoSanityCheck(bool ignoreGraphCacheWarning) {
		...
		var activeScene = EditorSceneManager.GetSceneAt(0);
		if (activeScene.name == "ExampleScene") {
		...
		var metaDataPath = MetaDataUtils.GetMetaDataPath(activeScene.path);
		Debug.Log("Meta data path waow " + metaDataPath);
		if(!MetaDataUtils.MetaDataExistsFor(activeScene.path)) {
			Log("Current map has no metadata. Create meta data with Ravenfield Tools -> Map -> Metadata editor");
		}
