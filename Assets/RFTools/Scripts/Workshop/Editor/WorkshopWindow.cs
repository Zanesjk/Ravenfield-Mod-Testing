...
public class WorkshopWindow : EditorWindow {
	...
	void SetupItemTags() {
		...
		if (maps.Count > 0) {
			...
			foreach(var map in maps) {
				if(MetaDataUtils.ReadMetaData(map.FullName, out MapMetaData metaData, MapMetaData.Default)) {
					...
					if(metaData.typePrimary != MapMetaData.MapType.Unknown) {
						this.steamworks.currentItem.tags.Add(metaData.typePrimary.ToString());
					}
					if (metaData.typeSecondary != MapMetaData.MapType.Unknown) {
						this.steamworks.currentItem.tags.Add(metaData.typeSecondary.ToString());
					}
					if (metaData.theme != MapMetaData.MapTheme.None) {
						this.steamworks.currentItem.tags.Add(metaData.theme.ToString());
					}
				}
			}
		}
	...
	void OnGUI() {
		...
		if (this.showErrorLabel) {
			...
			GUILayout.Space(10f);
			GUILayout.Label("Could not connect to Steam, please check the following:\n- Make sure you have a working internet connection\n- Make sure steam is running and is logged in.\n- Exit any other running steam game and try again", docStyle);
		}
	...
	void DisplayCurrentItemContent() {
		...
		DisplayModContent(this.exportedContent, "Exported Content", StageExportedContent, false);
		DisplayModContent(this.stagedContent, "Content to Publish", RemoveStagedContent, true);
	...
	void DisplayModContent(FolderModContent content, string title, DelContentClicked OnContentClicked, bool showDisallowed) {
		...
		if(GUILayout.Button("Browse")) {
			Debug.Log(content.path);
			Application.OpenURL($"file:///{content.path}");
			//EditorUtility.RevealInFinder(content.path);
		}
		...
		if (content.allMetaContent.Count > 0) {
			GUI.color = Color.grey;
			GUILayout.Space(10f);
			GUILayout.Label("Metadata:");

			foreach (FileInfo file in content.allMetaContent) {
				string label = file.Name;
		...
		if (showDisallowed && content.disallowedFiles.Count > 0) {
			GUI.color = Color.red;
			GUILayout.Space(10f);
			GUILayout.Label("Disallowed:");

			foreach (FileInfo file in content.disallowedFiles) {
				string label = file.Name;
