...
public class Weapon : MonoBehaviour {
	...
	public enum Difficulty { Auto = 0, Easy = 1, Challenging = 2, Hard = 3, Impossible = 4 };
	...
	public bool keepScriptsOnThirdPersonImposter = false;
	...
	[System.NonSerialized] public bool isLocked = false;
	...
	TimedAction stopFireLoop = new TimedAction(0.12f);
	bool fireLoopPlaying = false;

	TimedAction followupSpreadStayAction;
	float followupSpreadMagnitude = 0f;
	float followupSpreadDissipateRate = 1f;

	TimedAction heatStayAction;
	float heat = 0f;
	...
	protected virtual void Awake() {
		...
		this.chargeAction = new TimedAction(this.configuration.chargeTime);

		this.followupSpreadStayAction = new TimedAction(this.configuration.followupSpreadStayTime);
		this.followupSpreadDissipateRate = 1f/this.configuration.followupSpreadDissipateTime;
		this.heatStayAction = new TimedAction(this.configuration.cooldown*1.1f);
    }
    ...
    protected virtual void Start() {
        ...
        if (this.animator != null) {
            ...
            foreach (AnimatorControllerParameter parameter in this.animator.parameters) {
				...
				else if (parameter.nameHash == Animator.StringToHash("sight mode") && parameter.type == AnimatorControllerParameterType.Int) {
					this.notifySightMode = true;
				}
	...
	public class Configuration {
		...
		public Difficulty diffInfantry = Difficulty.Auto;
		public Difficulty diffInfantryGroup = Difficulty.Auto;
		public Difficulty diffGroundVehicles = Difficulty.Auto;
		public Difficulty diffAir = Difficulty.Auto;
		public Difficulty diffAirFastMover = Difficulty.Auto;
